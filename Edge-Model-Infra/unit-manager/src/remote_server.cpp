...
// Detection API RPC handlers
std::string rpc_detect_request(pzmq *_pzmq, const std::shared_ptr<pzmq_data> &raw)
{
    if (!g_detection_api) {
        return "{\"status\":\"error\",\"error\":{\"code\":-1,\"message\":\"Detection API not initialized\"}}";
    }
    return g_detection_api->handle_detect_request(raw->string());
}

std::string rpc_detection_status(pzmq *_pzmq, const std::shared_ptr<pzmq_data> &raw)
{
    if (!g_detection_api) {
        return "{\"status\":\"error\",\"error\":{\"code\":-1,\"message\":\"Detection API not initialized\"}}";
    }
    return g_detection_api->handle_status_request(raw->string());
}

std::string rpc_detection_setup(pzmq *_pzmq, const std::shared_ptr<pzmq_data> &raw)
{
    if (!g_detection_api) {
        return "{\"status\":\"error\",\"error\":{\"code\":-1,\"message\":\"Detection API not initialized\"}}";
    }
    return g_detection_api->handle_setup_request(raw->string());
}
...
void remote_server_work()
{
    ...
    g_detection_api = std::make_unique<DetectionAPI>();
    if (!g_detection_api->initialize()) {
        std::cerr << "Warning: Failed to initialize Detection API" << std::endl;
    }
    ...
    // Register detection API RPC actions
    sys_rpc_server_->register_rpc_action("detect_request",
                                         std::bind(rpc_detect_request, std::placeholders::_1, std::placeholders::_2));
    sys_rpc_server_->register_rpc_action("detection_status",
                                         std::bind(rpc_detection_status, std::placeholders::_1, std::placeholders::_2));
    sys_rpc_server_->register_rpc_action("detection_setup",
                                         std::bind(rpc_detection_setup, std::placeholders::_1, std::placeholders::_2));
}

void remote_server_stop_work()
{
    sys_rpc_server_.reset();
    g_detection_api.reset();
}
...
void unit_action_match(int com_id, const std::string &json_str)
{
    ...
    // Handle detection API requests
    if (work_id == "detection" || work_id == "ai-detection") {
        std::string action;
        error = doc["action"].get_string().get(action);
        if (!error) {
            std::string response;
            if (action == "detect") {
                response = g_detection_api ? g_detection_api->handle_detect_request(json_str) :
                          "{\"status\":\"error\",\"error\":{\"code\":-1,\"message\":\"Detection API not initialized\"}}";
            } else if (action == "status") {
                std::string task_id;
                auto data_obj = doc["data"];
                if (!data_obj["task_id"].get_string().get(task_id)) {
                    response = g_detection_api ? g_detection_api->handle_status_request(task_id) :
                              "{\"status\":\"error\",\"error\":{\"code\":-1,\"message\":\"Detection API not initialized\"}}";
                } else {
                    response = "{\"status\":\"error\",\"error\":{\"code\":-2,\"message\":\"Missing task_id\"}}";
                }
            } else if (action == "setup") {
                response = g_detection_api ? g_detection_api->handle_setup_request(json_str) :
                          "{\"status\":\"error\",\"error\":{\"code\":-1,\"message\":\"Detection API not initialized\"}}";
            } else {
                response = "{\"status\":\"error\",\"error\":{\"code\":-3,\"message\":\"Unknown action\"}}";
            }
            ...
            out_body["request_id"] = request_id;
            out_body["work_id"] = work_id;
            out_body["action"] = action;
            out_body["data"] = nlohmann::json::parse(response);
            std::string out = out_body.dump();
