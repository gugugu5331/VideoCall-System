...
graph TB
    ...
    subgraph Microservices["🎯 微服务层 Go + Gin"]
    ...
    subgraph DataLayer["💾 数据层"]
        PostgreSQL["🗄️ PostgreSQL<br/>(主数据库)<br/>用户/会议/参与者数据"]
        Redis["⚡ Redis<br/>(缓存/队列)<br/>Session/消息队列"]
        MongoDB["📊 MongoDB<br/>(AI数据)<br/>推理结果/分析数据"]
    ...
    AISvc --> ModelMgr
    AISvc --> InferEngine
    InferEngine --> InferCluster

    %% 微服务到数据层
    UserSvc --> PostgreSQL
    MeetingSvc --> PostgreSQL
    SignalSvc --> Redis
    MediaSvc --> PostgreSQL
    AISvc --> MongoDB
    NotifySvc --> Redis
