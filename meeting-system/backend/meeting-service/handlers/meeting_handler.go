...
func (h *MeetingHandler) CreateMeeting(c *gin.Context) {
	...
	if req.StartTime.Before(time.Now()) {
		response.BadRequest(c, "Start time cannot be in the past")
		return
	}
...
func (h *MeetingHandler) ListMeetings(c *gin.Context) {
	var req models.MeetingListRequest
	...
	meetings, total, err := h.meetingService.ListMeetings(userID.(uint), &req)
	if err != nil {
		logger.Error("Failed to list meetings", logger.Err(err))
		response.InternalServerError(c, "Failed to list meetings")
		return
	}

	response.Page(c, meetings, total, req.Page, req.PageSize)
}
