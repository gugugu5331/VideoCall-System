...
cleanup() {
    ...
    pkill -f "go run.*ai-service" || true
...
echo -e "${GREEN}[1/5] 启动用户服务 (HTTP:8080, gRPC:50051)...${NC}"
cd user-service
...
echo -e "${GREEN}[2/5] 启动会议服务 (HTTP:8082, gRPC:50052)...${NC}"
cd meeting-service
...
echo -e "${GREEN}[3/5] 启动信令服务 (HTTP:8081)...${NC}"
...
echo -e "${GREEN}[4/5] 启动媒体服务 (HTTP:8083)...${NC}"
cd media-service
...
echo -e "${GREEN}[5/5] 启动AI服务 (HTTP:8084)...${NC}"
cd ai-service
nohup go run main.go -config=../config/ai-service.yaml > logs/service.log 2>&1 &
AI_PID=$!
...
if curl -f -s http://localhost:8084/health > /dev/null 2>&1; then
    echo -e "${GREEN}✅ AI服务 HTTP (8084) - 运行正常${NC}"
else
    echo -e "${RED}❌ AI服务 HTTP (8084) - 未响应${NC}"
fi
...
echo "  - AI服务 HTTP:    http://localhost:8084"
echo ""
echo "健康检查: /health"
